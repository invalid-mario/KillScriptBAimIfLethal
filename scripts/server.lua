...
local function FindTargetHitboxAndAimPositionFirearm(target, mode, firearm, penetrationThresholdPercent, firePos)
    ...
    ---@type Hitbox
    local primary = list[primaryIndex]
    if primary then
        if CpuLimit.RemainingCpuTime >= 0.45 then
            local ok, mp = TryMultiPointHitFirearm(primary, firearm, penetrationThresholdPercent, firePos,
                targetOwnerId)
            if ok then return primary, mp end
        else
            if TrySinglePointHitFirearm(primary, firearm, penetrationThresholdPercent, firePos, targetOwnerId) then
                return primary, primary.Position
            end
        end
    end

    if mode == EReflexAimMode.OnlyHead then
        return nil, nil
    end
    ...
    for i = primaryIndex + 1, length do
...
---@param myTeam Team
---@return integer
local function GetSuccessfulHits(aimMode, successfulHitsThreshold, penetrationThresholdPercent, firearm, myTeam)
            ...
            if hb and h.EntryPower * 100 >= penetrationThresholdPercent then
                ...
                if IsAgentAlive(victim) and victim.IsVisible then
                    if victim.Team == myTeam then
                        ...
                        if (not wantHeadOnly) or (hb.BodyPart == EHitboxBodyPart.Head) then
                            score = score + 1
    ...
    if CurrentItem.IsFirearm then
        ...
        if threshold > 0 and not firearm.IsReloading and firearm.ClipAmmo > 0 then
            local percent = GetSuccessfulHits(NetworkState.AimMode, threshold,
                penetrationThresholdPercent, firearm, LocalAgent.Team)
